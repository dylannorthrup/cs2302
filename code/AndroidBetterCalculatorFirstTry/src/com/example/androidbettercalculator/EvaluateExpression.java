...
public class EvaluateExpression {
  ...
  private String improperExpression = "Improper Expression";
  ...
  public EvaluateExpression(Context context, String expression) {
    ...
    improperExpression = context.getString(R.string.strImproperEq);
  ...
  private void executePhase1(Stack<String> stack1, Stack<BigDecimal> stack2) 
    ...
    while(!stack1.empty()) {
      ...
      if (isOperator(token)) {
        ...
        throw new Exception(improperExpression);
      } else {
  ...
  private void doRootification(Stack<String> stack1, Stack<BigDecimal> stack2) throws Exception {
    debugInfo("*** doRootification: ENTERing function(" + stack1 + "," + stack2 + ")");
    ...
    // Take value we previously put onto stack2 off the stack
    BigDecimal bd2 = stack2.pop();
    debugInfo("PHASE 1: ROOTIFICATION => About to perform square root on this operand: " + bd2);
    // Convert it to a double, use Math.sqrt on that double, then turn the value back
    // into a BigDecimal
    bd2 = BigDecimal.valueOf(Math.sqrt(bd2.doubleValue()));
    // Now, take that value and push it back onto stack1
    stack2.push(bd2);
  }

  // Handle exponent operations
  private void doExponentiation(Stack<String> stack1, Stack<BigDecimal> stack2) throws Exception {
    ...
    // Handle special case where the next thing is actually a parenthesis
    // Also check to make sure stack isn't empty because if it is, bad things tend to happen
    debugInfo("PHASE 1: EXPONENTIATION => Checking for PARENCEPTION!", 3);
    if(!stack1.empty() && stack1.peek().contentEquals(")")) {
      debugInfo("PHASE 1: EXPONENTIATION => Next operator is a '('. Initiating PARENCEPTION!", 3);
      stack1.push(handleParenthesisExpression(stack1, stack1.pop()));
    }
    ...
    if(!stack1.isEmpty() && stack1.peek().contentEquals("-")) {
      ...
      if (! stack1.isEmpty() && ! isOperator(stack1.peek())) {
        debugInfo("PHASE 1: EXPONENTIATION => stack1 has something on it that doesn't look like an operator: " + stack1.peek());
        stack1.push("-");
      } else {
        debugInfo("PHASE 1: EXPONENTIATION => stack1 either doesn't have something on it, or whatever's on it looks like an operator");
        bd1 = bd1.negate();
      }
    }
    BigDecimal bd2 = stack2.pop();
    ...
    debugInfo("PHASE 1: EXPONENTIATION => equation: " + bd1 + "^" + bd2, 3);
    stack1.push("(");
    stack1.push("" + bd1);
    ...
    stack1.push(")");
    debugInfo("PHASE 1: EXPONENTIATION => Done with conversion to multiplication: " + bd1 + "^" + bd2, 3);
  }
